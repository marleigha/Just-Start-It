    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Rewards User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th>PointsRequired</th>
                    </tr>
                  </thead>
                  <tbody>
                    {users.rewards_user &&
                      Array.isArray(users.rewards_user) &&
                      users.rewards_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
          ...
          <>
            <p className={'block font-bold mb-2'}>Statistics User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.statistics_user &&
                      Array.isArray(users.statistics_user) &&
                      users.statistics_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/statistics/statistics-view/?id=${item.id}`,
                            )
                          }
                        >
                    ...
                    {users.tasks_user &&
                      ...
                      users.tasks_user.map((item: any) => (
                        ...
                        >
                          ...
                          <td data-label='hardness_level'>
                            {item.hardness_level}
                          </td>

                          <td data-label='deadline'>
                            {dataFormatter.dateTimeFormatter(item.deadline)}
                          </td>

                          <td data-label='urgency_level'>
                            {item.urgency_level}
                          </td>

                          <td data-label='point_value'>{item.point_value}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
