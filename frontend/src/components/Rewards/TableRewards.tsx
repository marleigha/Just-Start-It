...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/rewards/rewardsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureRewardsCols';
...
const TableSampleRewards = ({
  ...
  useEffect(() => {
    if (rewardsNotify.showNotification) {
      notify(rewardsNotify.typeNotification, rewardsNotify.textNotification);
    }
  }, [rewardsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/rewards/rewards-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {rewards && Array.isArray(rewards) && !showGrid && (
        <ListRewards
          rewards={rewards}
          loading={loading}
...
export default TableSampleRewards;
