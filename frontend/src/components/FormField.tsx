...
const FormField = ({ icons = [], ...props }: Props) => {
  ...
  const controlClassName = [
    ...
    props.isTransparent
      ? 'bg-transparent'
      : `${props.websiteBg ? `${bgWebsiteColor}  ` : bgColor} dark:bg-dark-800`,
    props.disabled ? 'bg-gray-200 text-gray-100 dark:bg-dark-900 disabled' : '',
    props.borderButtom
      ? `border-0 border-b ${
          props.diversity
            ? 'placeholder-primaryText border-primaryText'
            : ' placeholder-black   '
  ...
  return (
    <div className='mb-6 last:mb-0'>
      ...
      {props.help && (
        <div className='text-xs    text-gray-200 dark:text-dark-600 mt-1'>
          {props.help}
        </div>
      )}
    </div>
